...
CONFIG_BOT_TOKEN = "8687197135:AAGpeOBgHiUpiJg1_4arkYw-WOf6tdh3zh8"
CONFIG_LAMIX_API_KEY = "http://51.77.216.195/crapi/lamix/viewstats"
CONFIG_LAMIX_BASE_URL = "http://51.210.208.26/ints/agent/API"
CONFIG_ADMIN_ID = 8110991315
CONFIG_OTP_GROUP_ID = -1004356371507
...
ADMIN_ID = int(os.getenv("ADMIN_ID", str(CONFIG_ADMIN_ID or "8110991315")))
ADMINS = [ADMIN_ID]

OTP_GROUP_ID = int(os.getenv("OTP_GROUP_ID", str(CONFIG_OTP_GROUP_ID or "-1004356371507")))
...
def load_settings():
    ...
    default = {
        "active_panel": "lamix",
        "lamix_api_key": os.getenv("LAMIX_API_KEY", "").strip() or CONFIG_LAMIX_API_KEY.strip(),
        "lamix_base_url": os.getenv("LAMIX_BASE_URL", CONFIG_LAMIX_BASE_URL),
        "panel_url": f"https://t.me/Lamix_Number_bot?start={ADMIN_ID}",
        "allowed_services": ["Instagram","Facebook","WhatsApp","TikTok","Telegram","Discord","PayPal","Imo"],
        "otp_group_url": "https://t.me/+q5_5qBjZ1QJhZGY0",
        "channel_url": "https://t.me/+q5_5qBjZ1QJhZGY0",
...
def get_api_credentials():
    ...
    env_api_key = os.getenv("LAMIX_API_KEY", "").strip() or CONFIG_LAMIX_API_KEY.strip()
    configured_api_key = str(settings.get("lamix_api_key", "")).strip()
    return (env_api_key or configured_api_key,
            clean_base_url(settings.get("lamix_base_url","https://api.lamixnetwork.com")))
...
def bot_settings_keyboard():
    keyboard = [
        ...
        [make_reply_btn("SET OTP CHAT ID",    emoji_id="5231200819986047254", style="primary"),
         make_reply_btn("LAMIX CONFIG",       emoji_id="5350396951407895212", style="primary")],
        [make_reply_btn("BACK TO ADMIN",      emoji_id="5267490665117275176", style="primary")]
    ]
    return ReplyKeyboardMarkup(keyboard, resize_keyboard=True)

def lamix_config_keyboard():
    keyboard = [
        ...
        [make_reply_btn("SET ALLOWED SERVICES", emoji_id="5341715473882955310", style="primary"),
         make_reply_btn("VIEW LAMIX CONFIG",    emoji_id="5231200819986047254", style="primary")],
...
async def show_app_selection(update, context):
    ...
    # Build combined keyboard: Lamix apps + manual services
    async def _build_full_keyboard(top):
        lamix_rows = build_app_buttons_from_cache(top)
        manual_rows = build_manual_service_buttons()
        all_rows = lamix_rows + manual_rows
        if not all_rows:
            all_rows = []
    ...
    if err:
        ...
        if manual_rows:
            ...
            await status.edit_text(
                f'{get_tg_emoji("get_number_btn")} <b>SELECT APP TO GET NUMBER</b>\n━━━━━━━━━━━━━━━━━━━━━\n<blockquote>⚠️ Lamix unavailable — showing manual services</blockquote>',
                parse_mode="HTML",
        ...
        return
    if not top:
        # Show only manual services if Lamix fails
        manual_rows = build_manual_service_buttons()
        if manual_rows:
            manual_rows.append([InlineKeyboardButton("⚙️ CUSTOM RANGE", callback_data="custom_range",
                                                       api_kwargs={"style": "primary"})])
            await status.edit_text(
                f'{get_tg_emoji("get_number_btn")} <b>SELECT APP TO GET NUMBER</b>\n━━━━━━━━━━━━━━━━━━━━━\n<blockquote>⚠️ Lamix ranges unavailable — showing manual services</blockquote>',
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup(manual_rows))
        else:
            await status.edit_text(
...
# ==================== AUTO OTP MONITOR SECTION (LAMIXA) ====================
            ...
            if otps:
                ...
                for otp in otps:
                    ...
                    if (num in active_numbers and
                        ...
                        panel_url = settings.get("panel_url", f"https://t.me/Lamix_Number_bot?start={ADMIN_ID}")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "BACK TO ADMIN":
        ...
        context.user_data["lamix_config_mode"] = None
    ...
    if text == "LAMIX CONFIG" and context.user_data.get("bot_settings_mode") == "main" and is_admin(uid):
        context.user_data["lamix_config_mode"] = "main"
        await update.message.reply_text("📡 <b>LAMIX PANEL CONFIG</b>", parse_mode="HTML", reply_markup=lamix_config_keyboard())
        return
    ...
    # ==================== LAMIX CONFIG HANDLERS ====================
    if text == "SET API KEY" and context.user_data.get("lamix_config_mode") == "main" and is_admin(uid):
        context.user_data["lamix_config_mode"] = "set_api_key"
        s = load_settings()
        cur = s.get("lamix_api_key", "Not set")
        await update.message.reply_text(
            f"🔑 <b>SET LAMIX API KEY</b>\n\n<blockquote>Current: <code>{cur[:20]}...</code></blockquote>\n\nSend new API key:",
            parse_mode="HTML", reply_markup=cancel_keyboard())
        return

    if context.user_data.get("lamix_config_mode") == "set_api_key" and is_admin(uid):
        s = load_settings()
        s["lamix_api_key"] = text.strip()
        save_settings(s)
        await update.message.reply_text(
            f"✅ <b>LAMIX API KEY UPDATED!</b>\n<blockquote><code>{html.escape(text.strip()[:20])}...</code></blockquote>",
            parse_mode="HTML", reply_markup=lamix_config_keyboard())
        context.user_data["lamix_config_mode"] = "main"
        return

    if text == "SET BASE URL" and context.user_data.get("lamix_config_mode") == "main" and is_admin(uid):
        context.user_data["lamix_config_mode"] = "set_base_url"
        s = load_settings()
        cur = s.get("lamix_base_url", "Not set")
        await update.message.reply_text(
            f"🌐 <b>SET LAMIX BASE URL</b>\n\n<blockquote>Current: <code>{cur}</code></blockquote>\n\nSend new base URL (e.g. https://api.lamixnetwork.com):",
            parse_mode="HTML", reply_markup=cancel_keyboard())
        return

    if context.user_data.get("lamix_config_mode") == "set_base_url" and is_admin(uid):
        s = load_settings()
        s["lamix_base_url"] = text.strip().rstrip("/")
        save_settings(s)
        await update.message.reply_text(
            f"✅ <b>BASE URL UPDATED!</b>\n<blockquote><code>{html.escape(s['lamix_base_url'])}</code></blockquote>",
            parse_mode="HTML", reply_markup=lamix_config_keyboard())
        context.user_data["lamix_config_mode"] = "main"
        return

    if text == "SET ALLOWED SERVICES" and context.user_data.get("lamix_config_mode") == "main" and is_admin(uid):
        context.user_data["lamix_config_mode"] = "set_services"
        s = load_settings()
        cur = ", ".join(s.get("allowed_services", []))
        await update.message.reply_text(
            f"📋 <b>SET ALLOWED LAMIX SERVICES</b>\n\n<blockquote>Current:\n<code>{cur}</code></blockquote>\n\n"
            f"Send comma-separated list:\n<i>e.g. Instagram,Facebook,WhatsApp,TikTok</i>",
            parse_mode="HTML", reply_markup=cancel_keyboard())
        return

    if context.user_data.get("lamix_config_mode") == "set_services" and is_admin(uid):
    ...
    if text == "VIEW LAMIX CONFIG" and context.user_data.get("lamix_config_mode") == "main" and is_admin(uid):
        s = load_settings()
        api_key = s.get("lamix_api_key", "N/A")
        base_url = s.get("lamix_base_url", "N/A")
        ...
        msg = (
            f"📡 <b>LAMIX PANEL CONFIGURATION</b>\n━━━━━━━━━━━━━━━━━━━━━━\n\n"
        ...
        await update.message.reply_text(msg, parse_mode="HTML", reply_markup=lamix_config_keyboard())
        return
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data in ("back_apps", "back_services"):
        top = context.user_data.get("top_ranges_by_app") or (_ranges_cache.get("data") or {})
        manual_rows = build_manual_service_buttons()
        lamix_rows = build_app_buttons_from_cache(top) if top else []
        all_rows = lamix_rows + manual_rows
        all_rows.append([InlineKeyboardButton("\u2699\ufe0f CUSTOM RANGE", callback_data="custom_range",
                                               api_kwargs={"style": "primary"})])
        msg = f'{get_tg_emoji("get_number_btn")} <b>SELECT APP TO GET NUMBER</b>\n\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501'
